...
export const AuthProvider = ({ children }) => {
    ...
    const [userType, setUserType] = useState(null); // 'customer', 'vendor', 'admin'
    ...
    useEffect(() => {
        const initAuth = () => {
            ...
            if (token && savedUser) {
                ...
                if (savedUser.roles?.includes('ROLE_ADMIN')) {
                    setUserType('admin');
                } else if (savedUser.roles?.includes('ROLE_VENDOR')) {
                    setUserType('vendor');
                } else {
    ...
    const login = async (type, username, password) => {
        ...
        try {
            ...
            } else if (response.roles?.includes('ROLE_VENDOR')) {
                setUserType('vendor');
