...
export default function Dashboard() {
    ...
    const handleSearch = async (e) => {
        ...
        // For train name/number, just validate (search already happens live)
        if (searchType === 'name' && searchQuery.length < 2) {
            alert('Please enter at least 2 characters to search');
            return;
        }
        if (searchType === 'number' && searchQuery.length < 1) {
                    ...
                    <form onSubmit={handleSearch} className="max-w-3xl mx-auto">
                        <div className="flex flex-col md:flex-row gap-4">
                            <div className="flex-1 relative group">
                                ...
                                <input
                                    ...
                                    className="w-full pl-14 pr-12 py-5 rounded-2xl border-2 border-gray-100 bg-gray-50/50 focus:bg-white focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 outline-none text-lg font-medium transition-all shadow-sm placeholder-gray-400"
                                    value={searchQuery}
                            ...
                            {/* Only show button for PNR search - train search is automatic */}
                            {searchType === 'pnr' && (
                                <button
                                    type="submit"
                        ...
                        {searchType === 'pnr' ? (
                            <p className="text-center text-gray-400 mt-4 text-sm font-medium">
                                Try PNR: <span className="font-mono text-gray-600 bg-gray-100 px-2 py-1 rounded border border-gray-200 ml-1">1234567890</span>
                            </p>
                        ) : (
                            <p className="text-center text-gray-500 mt-4 text-sm font-medium h-6">
                                {searchQuery.length < 2
                                    ? <span className="opacity-50">✨ Start typing to see matching trains...</span>
                                    : isSearching
                                        ? <span className="flex items-center justify-center gap-2"><Loader2 className="w-3 h-3 animate-spin" /> Searching...</span>
                                        : searchResults.length > 0
                                            ? <span className="text-green-600 bg-green-50 px-3 py-1 rounded-full">✅ Found {searchResults.length} train{searchResults.length !== 1 ? 's' : ''}</span>
                                            : searchQuery.length >= 2
                                                ? <span className="text-red-500 bg-red-50 px-3 py-1 rounded-full">😕 No trains found</span>
                                                : ''
                                }
                            </p>
                        )}
