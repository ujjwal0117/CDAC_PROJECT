...
export default function MenuPage() {
    ...
    const handleImageUpload = async (e) => {
        ...
        try {
            ...
            setFormData(prev => ({ ...prev, imageUrl: uploaded.fileDownloadUri }));
        } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingItem) {
                const updated = await foodItemAPI.update(editingItem.id, payload);
                setFoodItems(prev => prev.map(item => item.id === updated.id ? updated : item));
            } else {
                const created = await foodItemAPI.create(payload);
