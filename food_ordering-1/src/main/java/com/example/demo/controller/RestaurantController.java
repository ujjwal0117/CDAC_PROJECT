...
public class RestaurantController {
    ...
    public ResponseEntity<RestaurantResponse> approveRestaurant(@PathVariable Long id) {
        return ResponseEntity.ok(RestaurantResponse.fromEntity(restaurantService.approveRestaurant(id)));
    }

    @PutMapping("/{id}/status")
    @PreAuthorize("hasAuthority('ROLE_ADMIN')")
    public ResponseEntity<RestaurantResponse> updateStatus(@PathVariable Long id, @RequestParam Boolean active) {
        return ResponseEntity.ok(RestaurantResponse.fromEntity(restaurantService.updateRestaurantStatus(id, active)));
    }

    @GetMapping("/admin/pending")
    @PreAuthorize("hasAuthority('ROLE_ADMIN')")
    public ResponseEntity<List<RestaurantResponse>> getPendingRestaurants() {
        List<Restaurant> restaurants = restaurantService.getPendingRestaurants();
        List<RestaurantResponse> response = restaurants.stream()
                .map(RestaurantResponse::fromEntity)
                .collect(Collectors.toList());
        return ResponseEntity.ok(response);
